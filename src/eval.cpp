...
Value evaluate(const Board& board) {
    int score = V_PAWN   * diff(board, PieceType::PAWN)
              + V_KNIGHT * diff(board, PieceType::KNIGHT)
              + V_BISHOP * diff(board, PieceType::BISHOP)
              + V_ROOK   * diff(board, PieceType::ROOK)
