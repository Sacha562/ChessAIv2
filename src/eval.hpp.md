...
Phase 0 is material-only (plus a tiny tempo bonus). PSQT, tapered eval, pawn
structure, mobility, and king safety arrive in Phase 1c (see [PLAN.md](../PLAN.md)
§17); NNUE in Phase 2.
...
- Piece values (`V_PAWN` … `V_QUEEN`, `V_TEMPO`) and the `diff` helper live in an
  anonymous namespace in `eval.cpp` — internal linkage, **not** public API.
