...
depth with a fresh [`Searcher`](search.hpp.md#class-searcher), accumulate the node
counts, and print the benchmark summary.
...
breaks. Each position uses its own local `stop` flag (never triggered here).
